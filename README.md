...
まずここを確認してください。`mock` だけなら LLM や pywinpty は不要ですが、Codex / Claude Code を実際に使う場合は Windows の対話セッション駆動が前提です。

### 共通

- Python 3.9+
- Git
- PowerShell または同等のシェル

### Windows で Codex / Claude Code を使う場合

- `pip install -r requirements.txt` で `pywinpty` を入れる
- `codex` が PATH または既定の場所から起動できる
- `claude` が PATH または既定の場所から起動できる
- Codex / Claude Code がログイン済み
- Claude Code は初回のみ `--dangerously-skip-permissions` の承認を済ませておく
...
Windows 以外では、現時点では `--engine mock` による Python pipeline smoke を主用途とします。実 engine の対話駆動は Windows + pywinpty 前提です。

## 動作確認

### 1. 環境確認

token を使わず、pywinpty と `codex` / `claude` の実行ファイル解決だけを確認します。

```powershell
python orchestrate.py doctor
python orchestrate.py doctor --engine codex
python orchestrate.py doctor --engine claude
```

`doctor` はログイン状態や Claude Code の初回承認までは確認しません。そこは実 engine smoke で確認します。

### 2. Python pipeline smoke

LLM を呼ばない `mock` で、orchestrator の Python pipeline と artifact 生成だけを確認します。これは Codex / Claude Code の起動確認ではありません。
...
Codex / Claude Code を実際に起動できるか確認します。最初は `--budget quick` と `--no-lit-search` で小さく回します。

```powershell
python orchestrate.py --engine codex --budget quick --no-lit-search --seed "接続確認: 簡単な仮説を1つ作る"
python orchestrate.py --engine claude --budget quick --no-lit-search --seed "接続確認: 簡単な仮説を1つ作る"
```
...
| オプション | 意味 | 使う場面 |
|---|---|---|
| `--engine mock` | LLM なしで Python pipeline と artifact 生成を確認 | 初回テスト、CI 的な確認 |
...
- `python orchestrate.py doctor` は実 engine を起動しません。pywinpty と実行ファイル解決だけを確認します。
- login 状態、Claude Code の初回承認、モデル名の正当性は実 run で確認します。
...
| engine が起動しない | `python orchestrate.py doctor` で pywinpty と codex / claude の実行ファイル解決を確認 |
| Windows で動かない | `pip install -r requirements.txt` で pywinpty を入れ、`python orchestrate.py doctor` を再実行 |
