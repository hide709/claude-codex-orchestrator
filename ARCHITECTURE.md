...
```text
Planner -> Workers -> Proximity -> Red-team -> Revise -> Verify -> Hard gate -> Arbiter -> Human
```
...
| artifact | 役割 | 正本 / view |
|---|---|---|
| `charter.json` | seed、制約、評価軸、レンズを固定する | 正本 |
...
| charter | run の目的、制約、評価軸、レンズを固定したもの。以後の工程は charter を前提に動く。 |
...
| 最短の棄却テスト(cheapest_kill) | その候補を一番安く潰せる確認。 |
