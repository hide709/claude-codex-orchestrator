#!/usr/bin/env python3
"""
conpty_spike.py — pywinpty 版。外部駆動 interactive runner が成立するかの最小検証(Windows)。

  echo : 子プロセスの stdout が PTY 経由で読めるか(ctypes 版で失敗した点)
  codex: codex を対話起動 → job1(seed)で report 生成 → job2 を注入して report 生成
         (= supervisor が job 単位で wake/inject/observe できるか)

使い方:
  python tools/conpty_spike.py echo
  python tools/conpty_spike.py codex "<codex.exe のフルパス>"
依存: pywinpty(stdlib 縛りを外して採用)。Windows 専用。
本番(orchestrate.py の InteractiveSessionRunner)と同じプロトコルで検証する:
  report は .json.tmp に書いてから .json へ rename(atomic)/ 完了判定は parse 可能な JSON。
"""
import importlib.util
import json
import os
import sys
import threading
import time
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent


def _require_winpty():
    """Windows + pywinpty が無ければ明示的に終了(生 ImportError で落とさない)。"""
    if os.name != "nt":
        print("このスパイクは Windows 専用です(ConPTY / pywinpty)。")
        sys.exit(2)
    if importlib.util.find_spec("winpty") is None:
        print("pywinpty が見つかりません: `pip install -r requirements.txt` を実行してください。")
        sys.exit(2)


class Reader:
    """PtyProcess の出力を別スレッドで読み続けてバッファに溜める(最新 256 チャンクのみ保持)。"""
    def __init__(self, proc, max_chunks=256):
        self.proc = proc
        self.max_chunks = max_chunks
        self._chunks = []
        self._lock = threading.Lock()
        threading.Thread(target=self._loop, daemon=True).start()

    def _loop(self):
        while True:
            try:
                data = self.proc.read(4096)
            except Exception:
                break
            if data:
                with self._lock:
                    self._chunks.append(data)
                    if len(self._chunks) > self.max_chunks:
                        del self._chunks[: len(self._chunks) - self.max_chunks]
            else:
                time.sleep(0.05)

    def text(self):
        with self._lock:
            return "".join(self._chunks)


def wait_json(p, timeout, poll=2):
    """parse 可能な JSON が書かれるまで待つ(存在だけでは部分書き込みを誤検知しうる)。"""
    p = Path(p)
    t0 = time.time()
    while time.time() - t0 < timeout:
        if p.exists():
            try:
                json.loads(p.read_text(encoding="utf-8", errors="replace"))
                return True
            except Exception:
                pass               # 書き込み途中 → 次のポーリングで再判定
        time.sleep(poll)
    return False


def mode_echo():
    from winpty import PtyProcess
    print("[echo] pywinpty で子プロセスを起動し、stdout を PTY 経由で読む…")
    proc = PtyProcess.spawn(
        [sys.executable, "-c", "import time; print('HELLO_CONPTY', flush=True); time.sleep(8)"],
        cwd=str(ROOT), dimensions=(30, 120))
    r = Reader(proc)
    for _ in range(10):
        time.sleep(1)
        if "HELLO_CONPTY" in r.text():
            break
    out = r.text()
    ok = "HELLO_CONPTY" in out
    print(f"[echo] alive={proc.isalive()} bytes={len(out)} marker={ok}")
    print("---- tail ----\n" + repr(out[-300:]))
    try:
        proc.terminate(force=True)
    except Exception:
        pass
    print("RESULT:", "PASS" if ok else "FAIL")


def _launch_argv(engine, exe, job):
    if engine == "codex":
        return [exe, "-c", "service_tier=flex", "-s", "workspace-write", "-a", "never", job]
    if engine == "claude":
        # 本番(orchestrate.py)と揃える。BypassPermissions 承認は初回一度きり(承認済み前提)。
        return [exe, "--dangerously-skip-permissions", job]
    raise ValueError(f"unknown engine {engine}")


def _job_text(engine, name, n):
    # 本番 directive と同じ .tmp → rename プロトコル
    return (f'queue/{engine}/reports/{name}.json.tmp に、説明やコードフェンス無しで '
            f'JSON {{"ok": true, "job": {n}}} だけを書き、書けたら '
            f'queue/{engine}/reports/{name}.json へ rename してください。完了したら SPIKE_DONE_{n} と出力。')


def mode_engine(engine, exe):
    from winpty import PtyProcess
    qrep = ROOT / "queue" / engine / "reports"
    qrep.mkdir(parents=True, exist_ok=True)
    for f in ("spike1.json", "spike2.json", "spike1.json.tmp", "spike2.json.tmp"):
        (qrep / f).unlink(missing_ok=True)        # stale な前回分(.tmp 含む)を掃除

    job1 = _job_text(engine, "spike1", 1)
    print(f"[{engine}] 起動: {exe} (interactive, seed=job1)")
    proc = PtyProcess.spawn(_launch_argv(engine, exe, job1), cwd=str(ROOT), dimensions=(40, 140))
    r = Reader(proc)

    print(f"[{engine}] step1(seed): spike1.json(parse 可能な JSON)を待つ(最大300s)…")
    ok1 = wait_json(qrep / "spike1.json", 300)
    print("  step1:", "PASS" if ok1 else "FAIL")

    ok2 = False
    if ok1:
        job2 = "次に " + _job_text(engine, "spike2", 2)
        print(f"[{engine}] step2(inject): job2 をテキスト注入 → Enter を別キーで送る…")
        proc.write(job2)
        time.sleep(1.5)
        proc.write("\r")                      # Enter を paste と分離して送る
        ok2 = wait_json(qrep / "spike2.json", 150)
        if not ok2:
            print("  (retry Enter: \\r\\n)")
            proc.write("\r\n")
            ok2 = wait_json(qrep / "spike2.json", 120)
        print("  step2:", "PASS" if ok2 else "FAIL")

    out = r.text()
    try:
        proc.terminate(force=True)
    except Exception:
        pass
    for f in ("spike1.json", "spike2.json", "spike1.json.tmp", "spike2.json.tmp"):
        (qrep / f).unlink(missing_ok=True)        # 後始末(queue/ に残骸を残さない)
    print(f"---- {engine} TUI tail(800) ----\n" + out[-800:])
    print(f"RESULT[{engine}]: step1={'PASS' if ok1 else 'FAIL'} step2(inject)={'PASS' if ok2 else 'FAIL'}")


if __name__ == "__main__":
    for s in (sys.stdout, sys.stderr):
        try:
            s.reconfigure(encoding="utf-8", errors="replace")
        except Exception:
            pass
    _require_winpty()
    mode = sys.argv[1] if len(sys.argv) > 1 else "echo"
    if mode == "echo":
        mode_echo()
    elif mode in ("codex", "claude"):
        if len(sys.argv) < 3:
            print(f'usage: python tools/conpty_spike.py {mode} "<{mode}.exe path>"')
            sys.exit(1)
        mode_engine(mode, sys.argv[2])
    else:
        print("unknown mode")
